    ...
    try {
      ...
      return utils.doRequestWithOptions(opts, function(err, statusCode, response) {
        ...
        if ((response != null ? response.status : void 0) === 404) {
          if (options.debug) {
            console.log(`host ${options.hostname} not found in ambari server cluster.node_add`);
          }
          opts['method'] = 'POST';
          if (typeof this.log === "function") {
            this.log({
              message: `Register host ${options.hostname} via API`,
              level: 'INFO',
          ...
          if (typeof this.log === "function") {
            this.log({
              message: `Host ost ${options.hostname} already exit via API`,
              level: 'INFO',
              module: 'ryba-ambari-actions/hosts/component_stop  '
            });
          }

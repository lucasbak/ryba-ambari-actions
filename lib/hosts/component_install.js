  ...
  try {
    ...
    return utils.doRequestWithOptions(opts, function(err, statusCode, response) {
      ...
      try {
        ...
        if (!(response['HostRoles']['state'] === 'INIT')) {
          if ((ref = response['HostRoles']['state']) === 'INSTALLED' || ref === 'STARTED' || ref === 'STOPPED') {
            return do_end();
          }
          if ((ref1 = response['HostRoles']['desired_state']) === 'STARTED' || ref1 === 'STOPPED') {
            return do_end();
          }
        }
        opts['method'] = 'PUT';

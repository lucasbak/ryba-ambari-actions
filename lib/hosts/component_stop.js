  ...
  try {
    ...
    return utils.doRequestWithOptions(opts, function(err, statusCode, response) {
      try {
        ...
        status = false;
        if (options.debug && response['HostRoles']['state'] === 'INSTALLED') {
          console.log(`component ${options.component_name} already ins STOPPED state`);
        }
        if ((response['HostRoles']['state'] === 'INSTALLED') && (response['HostRoles']['desired_state'] === 'INSTALLED')) {
          return do_end();
        }
        opts['method'] = 'PUT';
        ...
        if (options != null) {
          options.log({
            message: `Stopping Service ${options.component_name} via API`,
            level: 'INFO',
